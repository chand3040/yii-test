...
class UserDefaultListing extends CActiveRecord
{
    ...
    */
    public function rules()
    {
        ...
        return array(
            array(
                'user_default_listing_category_id, user_default_listing_lookingfor_id, user_default_listing_limit_viewing_id, user_default_listing_thumbnail, user_default_listing_title, user_default_listing_what_is_it, user_default_listing_summary, user_default_listing_details, user_default_listing_financial_table_status, user_default_profiles_id, user_default_listing_approvedate',
                'required'
            ),
            array(
                'user_default_listing_category_id, user_default_listing_lookingfor_id, user_default_listing_limit_viewing_id, user_default_listing_notification_frequency, user_default_listing_days_active, user_default_listing_days_inactive, user_default_listing_page_hits, user_default_profiles_id, user_default_listing_likes, user_default_listing_dislikes, user_default_listing_total_votes',
                'numerical',
                'integerOnly' => true
            ),
            array(
                'user_default_listing_thumbnail, user_default_listing_access_period',
                'length',
                'max' => 45
            ),
            array(
                'user_default_listing_title, user_default_listing_approvedate',
                'length',
                'max' => 100
            ),
            array(
                'user_default_listing_financial_table_status, user_default_listing_submission_status',
                'length',
                'max' => 1
            ),
            array(
                'user_default_listing_table_currency_code',
                'length',
                'max' => 3
            ),
            array(
                'user_default_listing_want, user_default_listing_keywords',
                'length',
                'max' => 255
            ),
    ...
    */
    ...
    public static function homeSearch($keyword, $searchCriteria, $searchViewOffset, $searchViewLimit, $orderBy)
    {
        ...
        if ($searchCriteria == 'all') {
            ...
            if ($Data) {
                ...
                $ids1 = rtrim($ids, ',');
                $Criteria->addInCondition('user_default_profiles_id', array(
                    $ids1
                ), $operator = 'OR');
            }
        ...
        $Criteria->addCondition("user_default_listing_submission_status='1'", $operator = 'AND');
    ...
    public static function allListingData($searchListingTitle, $searchViewOffset, $searchViewLimit, $orderBy, $searchCategory, $searchLookingFor, $searchViewingLimitByCountry, $searchKeyword, $searchSortCondtion, $searchSortByValue)
    {
        // $Criteria = new CDbCriteria();
        ...
        if ($searchKeyword != 'Nostr' && ($searchKeyword != '')) {
            $sql .= ' AND user_default_listing_keywords LIKE "%' . $searchKeyword . '%"';
        ...
        if ($searchSortCondtion != 'Nostr' && ($searchSortCondtion != '') && ($searchSortByValue != 'Nostr') && ($searchSortByValue != '')) {
            if ($searchSortCondtion == 'key') {
                ...
                $sql .= ' ORDER BY FIELD(user_default_listing_keywords, "' . $searchSortByValue . '") DESC';
            }
            if ($searchSortCondtion == 'listingTitle') {
                ...
                $sql .= ' ORDER BY FIELD(user_default_listing_title, "' . $searchSortByValue . '") DESC';
            }
        }
        if ($orderBy == 'user_default_listing_approvedate ASC') {
            //$Criteria->order = ' user_default_listing_approvedate ASC';
