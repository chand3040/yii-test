...
if ($userType == "user") {
    ...
    $sql .= " LEFT JOIN ( SELECT COUNT(*) AS count_member, A.user_default_profession
                                FROM {{profiles}} A 
                                WHERE A.user_default_business_user_type = 'user'
    ...
    <div class="info_panel_table info_panel_table_search"
         style="position: relative;top: -80px;margin-bottom: -34px;left: 400px;width: 480px;">
        ...
        <div class="info_panel_row">
            ...
            <div class="info_panel_cell">
                ...
                foreach ($usersProfession as $userProfession) {
                    ...
                    <p><?php
                        echo $logguedInByProfession = count(CommonClass::getLoggedInUsersByCriteria("user_default_business_user_type = 'user' AND user_default_profession = '{$userProfession['profession_id']}'"));
                        ?>
                    </p>
                    <?php
...
<?php
    ...
    $sql .= " FROM {{business}}";
    $sql .= " WHERE user_default_business_user_type = 'business'; ";
    $businessCount = Yii::app()->db->createCommand($sql)->queryAll();
    $businessCount = $businessCount[0]['business_count'];

    //$businessLogguedIn = count(CommonClass::getLoggedInUsersByCriteria("user_default_business_user_type = 'business'"));

    ?>

    <!-- info_panel_table_search -->
    <div class="info_panel_table info_panel_table_search"
         style="position: relative;margin-bottom: 36px;top: -80px;left: 400px;width: 480px;">
...
<div class="user_listing_search" style="margin-top: -70px !important;"><br>
    <a href="<?php echo $this->createUrl('/admin/businessuser/create'); ?>" class="button blue">+Add</a>
    <?php
    ...
    $this->widget('zii.widgets.grid.CGridView', array(
        ...
        'columns' => array(
            array(
                ...
                'value' => 'CHtml::link($data->user_default_business_username, Yii::app()->createUrl("/admin/businessuser/update",array("id"=>$data->primaryKey)),array("class"=>"full-link"))',
                'type' => 'raw',
            ),
            array(
                'header' => 'Email',
                ...
                'type' => 'raw',
                'value' => 'CHtml::link($data->user_default_business_email, Yii::app()->createUrl("/admin/businessuser/update",array("id"=>$data->primaryKey)),array("class"=>"full-link"))',
            ),
            array(
                'header' => 'Title',
                'name' => 'user_default_business_title',
                'type' => 'raw',
                'value' => 'CHtml::link($data->user_default_business_title, Yii::app()->createUrl("/admin/businessuser/update",array("id"=>$data->primaryKey)),array("class"=>"full-link"))',
            ),
            /*   array(
                   'header' => 'Address',
               ...
               array(
                   ...
                   'type' => 'raw',
                   'value' => 'CHtml::link(Country::model()->findByPk($data->user_default_country)->user_default_country_name, Yii::app()->createUrl("/admin/businessuser/update",array("id"=>$data->primaryKey)),array("class"=>"full-link"))',
               ),*/
            array(
                'header' => 'Account status',
                'name' => 'user_default_business_status',
                'type' => 'raw',
                'value' => 'CHtml::link( CommonClass::getUserStatusLabel($data->user_default_business_status), Yii::app()->createUrl("/admin/businessuser/update",array("id"=>$data->primaryKey)),array("class"=>"full-link"))',
            ),
            array(
                'header' => 'Date',
                'name' => 'user_default_business_rdate',
                'type' => 'raw',
                'value' => 'CHtml::link(CommonClass::getUkDate($data->user_default_business_rdate), Yii::app()->createUrl("/admin/businessuser/update",array("id"=>$data->primaryKey)),array("class"=>"full-link"))',
            ),
