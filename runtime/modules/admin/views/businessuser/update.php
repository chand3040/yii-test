...
<?php
$this->breadcrumbs = array($model->user_default_business_user_type == 'business' ? 'user' : 'default user account');
?>
...
<div class="update-profile-pic profile-pic">
    ...
    <div class="pro-photo" style="float: left; width: 44%;">
          ...
          <div class="profile-detail" style="width: 55%;">
            ...
            <?php 
            $prev_member = Member::prev_member($model->user_default_business_id);
            $next_member = Member::next_member($model->user_default_business_id);
            ...
            <div class="memberNavigation" style="width: 45%;"> 
                <?php if(is_object($prev_member)){  ?>
                    <aside class="prevLink floatLeft">
                        <?php echo CHtml::link('Previous', $this->createUrl('/admin/member/update/id/'.$prev_member->user_default_business_id)); ?>    
                    </aside>
                <?php } ?>

                <?php if(is_object($next_member)){ ?>
                    <aside class="nextLink floatRight">
                        <?php echo CHtml::link('Next>>', $this->createUrl('/admin/member/update/id/'.$next_member->user_default_business_id)); ?>
                    </aside>
                <?php } ?>
            </div>
            <?php
