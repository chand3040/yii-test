...
$this->breadcrumbs=array(
    ...
    'select listing action'=>array('/listing/selectlisting/listid/'.$_REQUEST['listid']),
    'marketing data'
);
?>
<style>
    ...
    .home-slider-wrap , #rightbar { display:none !important;}
    #world-map {margin-left: 0px !important;
        margin: 0 auto;
...
<div class="clear"></div>
<div class="full-width-content"><!-- /full-width-content start -->

    <?php if ($listingMarketing) { ?>
        <div class="submit_listing_box"
            <?php
            if( $_REQUEST['graphType'] =="geolocation")
            {
                echo 'style="height: 750px;"';
            }
            else
            {
                echo 'style="height: 680px;"';
            }
            ?>
        >
                ...
                if ($totalRecords > 1) {
                    ...
                    <ul id="navlist" class="topNav" style="font-size: 1.2em; /*margin-top: 10px;*/ float: right;">
                        <?php if ($prevRecord || $prevRecord == 0) : ?>
                            <a href="<?php echo Yii::app()->createUrl('/listing/marketingdata/listid/' . $listid . '?offset=' . $prevRecord); ?>"
                               title="Goto previous marketing question">&#60;&#60; </a></li>
                        <?php endif; ?> &nbsp;&nbsp;&nbsp;&nbsp;

                        <?php if ($nextRecord) : ?>
                            <li>
                                <a href="<?php echo Yii::app()->createUrl('/listing/marketingdata/listid/' . $listid . '?offset=' . $nextRecord); ?>"
              ...
              rows="1"><?php echo $listingMarketing->user_default_listing_marketing_question; ?></textarea>
                ...
                <div class="sl-financial">
                    ...
                    <label class="clearfix">Number of days active
                        ...
                        $totalVotes = MarketingData::getTotalVotes($questionId, $start_date, $end_date, '', $userType);
                ...
                if ($graphType != 'geolocation') {
                    ...
                    $markers = array();
                    if ($voteType == 'Yes')
                        SharedFunctions::setLatLngUserVotes($markers, MarketingData::getYesVoteUsersAddressInfo($questionId, $start_date, $end_date, '', $userType), '#CF0');
                    else if ($voteType == 'Maybe')
                        SharedFunctions::setLatLngUserVotes($markers, MarketingData::getMaybeVoteUsersAddressInfo($questionId, $start_date, $end_date, '', $userType), '#CFF');
                    else if ($voteType == 'No')
                        SharedFunctions::setLatLngUserVotes($markers, MarketingData::getNoVoteUsersAddressInfo($questionId, $start_date, $end_date, '', $userType), '#F5A');
                    else {
                        SharedFunctions::setLatLngUserVotes($markers, MarketingData::getYesVoteUsersAddressInfo($questionId, $start_date, $end_date, '', $userType), '#CF0');
                        SharedFunctions::setLatLngUserVotes($markers, MarketingData::getMaybeVoteUsersAddressInfo($questionId, $start_date, $end_date, '', $userType), '#CFF');
                        SharedFunctions::setLatLngUserVotes($markers, MarketingData::getNoVoteUsersAddressInfo($questionId, $start_date, $end_date, '', $userType), '#F5A');
                ...
                </div><div class="nav-button" style="position: relative;
    float: right;">
...
<?php if ($graphType == 'geolocation') { ?>
    <script>
        ...
        jQuery('.world-map-container')
            .show()
            .css({'margin-top': '0px'});
        jQuery('#world-map').css({height: '450px', 'margin-left': '34px'});
        var markers = {};
