...
<div>
    <?php
    $this->renderPartial('//../modules/listing/views/layouts/default_slider', array('listing_category_id' => 5));
    ?>
</div>
...
<div id="tabs_container">
    <ul id="sign-up-tabs">
        <li class="active">
            <a href="#taba" title="Full list of ALL Business Ideas entries">Science &#38;
                Tech<br/>( <?php echo $total_posts; ?> )</a>
        </li>
        <li>
...
<div id="taba" class="sign-up-tab_content" style="display: block;">
        ...
        if ($posts) {
            foreach ($posts as $post) {
                ...
                <tr onmouseover="ChangeColor<?php echo $color; ?>(this, true);"
                    ...
                    <td width="50%"> <?php  if($post->user_default_listing_summary!="") { $count =  strlen($post->user_default_listing_summary);
                                        if($count>145){
    ...
    <?php }else{?>
        <table border="0" bordercolor="#fff" style="background-color:#fff; cursor:pointer" width="100%" cellpadding="1"
               cellspacing="2">
            <td class="noteTemp">There are no science and technology currently available</td>
        </table>
    <?php } ?>
</div>
