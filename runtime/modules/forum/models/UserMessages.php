...
class UserMessages extends CActiveRecord
{
	...
	/*public function rules()
	{
		...
		return array(
			array('user_default_listing_id, subject, message, user_id, likes_total, dislikes_total, attachement, created_date', 'required'),
			array('user_default_listing_id, user_id, likes_total, dislikes_total', 'numerical', 'integerOnly'=>true),
			array('subject', 'length', 'max'=>250),
			array('attachement', 'length', 'max'=>255),
			array('is_spam, first_message', 'length', 'max'=>1),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_default_listing_id, subject, message, user_id, likes_total, dislikes_total, attachement, is_spam, first_message, created_date', 'safe', 'on'=>'search'),
		);
	}*/
	...
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'user_default_listing_id' => 'Blist',
			'subject' => 'Subject',
			'message' => 'Message',
			'user_default_profiles_id' => 'User',
    ...
    public function getPostComments($comment_id){
       ...
       // $sql .= " WHERE pc.comment_id = $comment_id";
		...
		$sql = "SELECT * from {{profiles_messages}} where parent_comment_id!='0' GROUP BY `parent_comment_id` ORDER BY created_date ASC";
    ...
    public static function getUserStats($listingId){
        ...
        $sql = "SELECT u.user_default_id as user_id, u.user_default_username as username,u.user_default_profile_image,u.user_default_first_name,u.user_default_surname, sum(likes_total) as like_total, sum(dislikes_total) as dislike_total, (sum(likes_total) - sum(dislikes_total)) as user_reputation ";
        $sql .= "FROM user_default_profiles_messages c ";
        $sql .= "INNER JOIN user_default_profiles u ";
        $sql .= "ON c.user_default_profiles_id = u.user_default_id ";
        $sql .= "WHERE c.user_default_listing_id =".$listingId ;
        $sql .= " GROUP BY c.user_default_profiles_id";
