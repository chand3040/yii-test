...
class MymessagesController extends Controller
{
    ...
    public function actionlikeMessage(){
        ...
        $criteria->addCondition("user_default_profiles_id = {$authenticatedUserId}");
        ...
        if ( ! ($likeMessage instanceof LikeMessage) ){
            ...
            $likeMessage->user_default_profiles_id = $authenticatedUserId;
    ...
    public function actionUploadAttachement(){
        ...
        if( ! $error ){
            ...
            if( (!$error) && ( !in_array($attachement['type'], MessageClass::$allowedUploadType)) ){

                $error = TRUE;
                $message = "Attachment type not allowed.";
            }

            if( (!$error) && ( $attachement['size'] > MessageClass::$maxUploadFile) ){
        ...
        if( !$error ){
            ...
            $fileToUploadPath = MessageClass::$uploadDirectoryPath.time().".".$attachement['name'];
